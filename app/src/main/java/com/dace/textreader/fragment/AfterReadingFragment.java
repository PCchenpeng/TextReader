...
import com.dace.textreader.adapter.AfterReadingAdapter;
 ...
 * ==============================================================================
 */
public class AfterReadingFragment extends Fragment {

    private static final String url = HttpUrlPre.HTTP_URL + "/select/feeling/list?";
    ...
    private List<AfterReadingBean> mList = new ArrayList<>();
    private AfterReadingAdapter adapter;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.fragment_after_reading, container, false);
    ...
    private void initEvents() {
        ...
        adapter.setOnItemClickListen(new AfterReadingAdapter.OnItemClickListen() {
            @Override
    ...
    private void initView() {
        frameLayout = view.findViewById(R.id.frame_after_reading_fragment);

        refreshLayout = view.findViewById(R.id.smart_refresh_after_reading_fragment);
        refreshLayout.setRefreshHeader(new ClassicsHeader(mContext));
        refreshLayout.setRefreshFooter(new ClassicsFooter(mContext));

        recyclerView = view.findViewById(R.id.recycler_view_after_reading_fragment);
        LinearLayoutManager layoutManager = new LinearLayoutManager(mContext,
                LinearLayoutManager.VERTICAL, false);
        recyclerView.setLayoutManager(layoutManager);
        adapter = new AfterReadingAdapter(mContext, mList);
        ...
        tv_one.setText("在阅读时");
        tv_two.setText("可添加读后感形成笔记");
        tv_three.setText("");
        tv_three.setVisibility(View.GONE);

        tv_sure = view.findViewById(R.id.tv_sure_after_reading_fragment);
        iv_practice = view.findViewById(R.id.iv_practice_after_reading_fragment);
    }
            ...
            if (200 == jsonObject.optInt("status", -1)) {
                ...
                if (array.length() == 0) {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        AfterReadingBean bean = new AfterReadingBean();
                        bean.setEssayId(object.optLong("essayid", -1));
                        bean.setEssayType(object.optInt("type", -1));
                        bean.setEssayTitle(object.getString("title"));
                        bean.setContent(object.getString("feeling"));
                        if (object.getString("feelingTime").equals("")
                                || object.getString("feelingTime").equals("null")) {
                            bean.setDate("2018-01-01");
                        } else {
                            bean.setDate(DateUtil.time2YMD(object.getString("feelingTime")));
                        }
                        bean.setSelected(false);
                        bean.setEditor(isEditor);
                        mList.add(bean);
                    }
                    adapter.notifyDataSetChanged();
    ...
    private static class GetData
            extends WeakAsyncTask<String, Void, String, AfterReadingFragment> {

        protected GetData(AfterReadingFragment fragment) {
            super(fragment);
        }

        @Override
        protected String doInBackground(AfterReadingFragment fragment, String[] strings) {
            try {
        ...
        @Override
        protected void onPostExecute(AfterReadingFragment fragment, String s) {
            if (s == null) {
                fragment.errorData();
            } else {
