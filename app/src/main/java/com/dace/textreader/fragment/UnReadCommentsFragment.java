...
public class UnReadCommentsFragment extends Fragment {

    private static final String url = HttpUrlPre.HTTP_URL + "/me/unreadComments/index?";
    ...
    private List<UnReadComment> mList = new ArrayList<>();
    private UnReadCommentAdapter adapter;
    ...
    private void initEvents() {
        ...
        adapter.setOnReplyClickListener(new UnReadCommentAdapter.OnUnReadCommentReplyClickListener() {
            @Override
            public void onItemClick(int position) {
                replyDialog(position);
            }
        });
        adapter.setOnArticleClickListener(new UnReadCommentAdapter.OnUnReadCommentArticleClickListener() {
            @Override
            public void onItemClick(int position) {
...
//        startActivityForResult(intent, 0);
    ...
    private void replyDialog(int position) {
        final long essayId = mList.get(position).getEssayId();
        final int essayType = mList.get(position).getEssayType();
        final int userId = mList.get(position).getCommentUserId();
        final long commentId = mList.get(position).getCommentId();
        final String essayTitle = mList.get(position).getEssayTitle();
        NiceDialog.init()
                        ...
                        tv_commit.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                if (TextUtils.isEmpty(str)) {
                                    ...
                                    new CommitCommentData(UnReadCommentsFragment.this)
                                            .execute(commentUrl,
    ...
    private static class GetCommentData
            extends WeakAsyncTask<String, Integer, String, UnReadCommentsFragment> {

        protected GetCommentData(UnReadCommentsFragment fragment) {
            super(fragment);
        }

        @Override
        protected String doInBackground(UnReadCommentsFragment fragment, String[] params) {
            try {
        ...
        @Override
        protected void onPostExecute(UnReadCommentsFragment fragment, String s) {
            ...
            if (jsonObject.optInt("status", -1) == 200) {
                ...
                if (jsonArray.length() == 0) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        UnReadComment comment = new UnReadComment();
                        ...
                        if (json.getString("originalCommenterId").equals("") ||
                                json.getString("originalCommenterId").equals("null")) {
                        ...
                        comment.setEssayTitle(json.getString("essayTitle"));
                        mList.add(comment);
                    }
                    adapter.notifyDataSetChanged();
                    markHasRead();
    ...
    private static class CommitCommentData
        ...
        protected CommitCommentData(UnReadCommentsFragment fragment) {
            super(fragment);
        }
    ...
    private void initView() {
        ...
        adapter = new UnReadCommentAdapter(mContext, mList);
    ...
    private static class MarkHasRead
            extends WeakAsyncTask<String, Void, String, UnReadCommentsFragment> {

        protected MarkHasRead(UnReadCommentsFragment fragment) {
            super(fragment);
        }

        @Override
        protected String doInBackground(UnReadCommentsFragment fragment, String[] strings) {
            try {
                OkHttpClient client = new OkHttpClient();
