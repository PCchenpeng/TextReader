...
public class UnReadCommentAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private Context mContext;
    private List<UnReadComment> mList;

    public UnReadCommentAdapter(Context context, List<UnReadComment> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        final UnReadComment unReadComment = mList.get(position);
        ...
        ((ViewHolder) holder).tv_essayTitle.setText(unReadComment.getEssayTitle());
        ((ViewHolder) holder).iv_head.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                turnToUserHomePage(unReadComment.getReplyUserId());
            }
        });
        ...
        ((ViewHolder) holder).ll_essay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mOnItemArticleClickListener != null) {
                    mOnItemArticleClickListener.onItemClick(position);
                }
            }
        });
    }
     ...
     * @param userId
     */
    private void turnToUserHomePage(int userId) {
        if (userId != -1) {
    ...
    //自定义监听事件
    public interface OnUnReadCommentReplyClickListener {
        void onItemClick(int position);
    }

    private OnUnReadCommentReplyClickListener mOnItemClickListener;

    public void setOnReplyClickListener(OnUnReadCommentReplyClickListener listener) {
        mOnItemClickListener = listener;
    }

    //自定义监听事件
    public interface OnUnReadCommentArticleClickListener {
        void onItemClick(int position);
    }

    private OnUnReadCommentArticleClickListener mOnItemArticleClickListener;

    public void setOnArticleClickListener(OnUnReadCommentArticleClickListener listener) {
        mOnItemArticleClickListener = listener;
    }
