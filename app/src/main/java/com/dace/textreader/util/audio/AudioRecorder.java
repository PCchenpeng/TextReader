package com.dace.textreader.util.audio;
 ...
 * Created time 2018/12/20 0020 上午 10:23.
 * Version   1.0;
 * Describe :  录音工具类
 * History:
 * ==============================================================================
...
public class AudioRecorder {
    ...
    public void startRecord() {
        ...
        if (status == Status.STATUS_NO_READY || audioRecord == null) {
            MyToastUtil.showToast(mContext, "录音尚未初始化,请检查是否禁止了录音权限~");
        }
        ...
        String currentFileName = fileName + DateUtil.getTime();
...
//        return Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + fileName + ".pcm";
        return mContext.getFilesDir() + "/" + fileName + ".pcm";
    }
...
//        return Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + fileName + ".wav";
    ...
    private void makePCMFileToWAVFile() {
        new Thread() {
            @Override
            public void run() {
                if (PcmToWav.makePCMFileToWAVFile(getPcmFileAbsolutePath(fileName),
                        getWavFileAbsolutePath(fileName), true)) {
                    //操作成功
                } else {
                    //操作失败
